...
RSpec.describe "PATCH /v1/payments/:payment_id" do
  subject { patch "/v1/payments/#{payment.id}", headers: default_headers.merge(headers), params: payload.to_json }
  ...
  context "when current session owner is user" do
    ...
    context "when need to nullify categories" do
      ...
      it "responds with 200" do
        ...
        expect(payment.reload.categories).to be_empty
      end
    end
  ...
  context "when someone else trying to update payment details" do
